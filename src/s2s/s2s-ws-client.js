 ...
 *   the caller creates/resumes it synchronously on the tap gesture and hands it
 *   here; otherwise it stays suspended (silent) after the mic/session awaits.
 *   A provided context is NOT closed on close() — the caller owns it.
 * @property {AudioNode} [outputNode] Where to route the TTS playback signal.
 *   When set (e.g. TalkingHead's `audioAnalyzerNode`), the playback worklet
 *   connects here instead of the context destination, so the host app can run
 *   the voice through its own gain/analysis/effects graph.
 * @property {string} [workletBaseUrl] Base URL (absolute or origin-relative)
 *   from which the `mic-capture.js` / `audio-playback.js` AudioWorklet modules
 *   are served. Defaults to "/worklets/". (The original resolved them relative
 *   to import.meta.url, which breaks once this module is bundled.)
...
export class S2sWsRealtimeClient extends EventTarget {
  ...
  constructor(options) {
    ...
    /** @type {boolean} Whether we created the AudioContext (and may close it). */
    this._ownsCtx = !options.audioContext;
    /** @type {WsStatus} */
    this._status = "idle";
  ...
  async _setupAudio() {
    ...
    // Worklet modules must stay separate files at a stable URL (they can't be
    // bundled), so the app tells us where they're served from.
    const base = new URL(this.options.workletBaseUrl ?? "/worklets/", window.location.href);
    ...
    // Route the voice into the host's graph when provided (e.g. TalkingHead's
    // analyzer -> speech gain -> reverb -> speakers), else straight out.
    playbackNode.connect(this.options.outputNode ?? ctx.destination);
  ...
  async close() {
    ...
    // Only close a context we created; a caller-provided one (e.g. shared with
    // the avatar renderer) must survive this session's teardown.
    if (this._ownsCtx) {
      try {
